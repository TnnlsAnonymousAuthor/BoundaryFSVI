...
- (5)  Furthermore, we provide a thorough interpretability analysis on how boundary context samples lead to these improvements.
...
- We also use 1k _lion_ images from [ImageNet](http://www.image-net.org/) as OOD data to perform _BOUNDARY CONTEXT ANALYSIS_ experiments in Section V. (See `data/imagenet1k-lion` folder).


## Checkpoints
Please download the checkpoints <u>[here](https://drive.google.com/file/d/1PBFsH2E6ehQrlVJiPOvFJIYEU1eNJGeg/view?usp=sharing)</u>, and place in the project as directory `/output`.
...
Experimental results in Sections IV-B~IV-E support that both cost functions can effectively enhance the BNN classifier's prediction accuracy, uncertainty calibration performance, and OOD detection capability. However, in some experimental settings (e.g., TinyImageNet detection task in Table IV), KL divergence cost function consistently outperforms another one.
...
The motivation of this work is that: **_"boundary samples"_** can better facilitate FSVI (see Fig. 1). Based on this, we propose a boundaryGAN-BNN joint training framework (Algorithm 1). It uses "boundary samples", generated by boundaryGAN, to improve the BNN's prediction accuracy, uncertainty calibration performance, and OOD detection ability. 
...
### Demo 4: Boundary context feature analysis

We contribute the performance improvements of our work to two key contributions of boundary samples:
...
📝 Script [`examples/contextFeatureAnalysis.ipynb`](examples/contextFeatureAnalysis.ipynb) provides a quick demo for visualizing the feature distribution of different context set $\mathcal{C}$.

**Complete Implementation Process:**

**_(Step 1: Training)_**

The BNN classifier trained on CIFAR10 Cat and Horse classes, using different context sets $\mathcal{C}$, have been saved in `output/kl_cost/cifar10CatHorse_(ctx){ctx_dataset}/model_epoch_40.pth`.

One can can retrain this BNN binary classifier by executing script [`run_inference.py`](run_inference.py) and [`run_gan_inference.py`](run_gan_inference.py). Note that the following parameters need to be modified:

```python
parser.add_argument('--epochs', type=int, default=40, help='number of epochs to train')
parser.add_argument('--num_classes', type=int, default=2, help='the # of classes')
parser.add_argument('--dataset', default='cifar10CatHorse')
parser.add_argument('--ctx_dataset', default='none', help='none | cifar10_augmentation | eurosat | cifar100_augmentation | gan (especially for run_gan_inference.py)')
```

**_(Step 2: Feature extraction)_**

Run the script [`contextFeatureGeneration.py`](contextFeatureGeneration.py) to extract the [UMAP](https://www.nature.com/articles/s43586-024-00363-x) feature representations of different samples using the following command:

```bash
python contextFeatureGeneration.py --ctx_dataset gan --out_dataset imagenet1k-lion
```

- `--ctx_dataset`: The context dataset used in training the BNN classifier, choices include `none`, `cifar10_augmentation`, `eurosat`, `cifar100_augmentation`, and `gan`.
- `--out_dataset`: The OOD dataset to be analyzed, choices include `imagenet1k-lion` and `imagenet`.

**_(Step 3: Feature visualization)_**

Run the script [`examples/contextFeatureAnalysis.ipynb`](examples/contextFeatureAnalysis.ipynb) to visualize the extracted features.

<details>
  <summary><b>Click here for the results! (see Figs. 5 and 6 for details)</b></summary>

<b>Boundary samples’ decision border discrimination capability. </b>

![Alts](figs/Fig5.png)


<b>Boundary samples’ OOD detection extension capability. </b>
...
<details>
  <summary><b>Click here for the results in Table I.</b></summary>

![Alts](figs/TableI.png)
...
<details>
  <summary><b>Click here for the results in Table II.</b></summary>

![Alts](figs/TableII.png)

</details>

<details>
  <summary><b>Click here for the results in Appendix Fig. S2.</b></summary>

<b>ECE calibration plots for different methods on SVHN dataset.</b>
...
<b>ECE calibration plots for different methods on CIFAR10 dataset.</b>
...
<details>
  <summary><b>Click here for the results in Table III.</b></summary>

![Alts](figs/TableIII.png)

</details>

<details>
  <summary><b>Click here for the results in Table IV.</b></summary>

![Alts](figs/TableIV.png)
...
<details>
  <summary><b>Click here for the results in Fig. 4.</b></summary>

<b>ODIN detection performance (All metrics, i.e., TNR95, AUC, et al., are the higher the better $\uparrow$) of different methods on six OOD datasets.</b>
![Alts](figs/Fig4.png)

</details>

<details>
  <summary><b>Click here for the results in Table S2.</b></summary>
